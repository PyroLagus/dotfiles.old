...
HASH=$(curl ftp://ftp.mozilla.org/pub/firefox/nightly/latest-mozilla-central/firefox-$VERSION.en-US.linux-$ARCH.checksums | grep "firefox-$VERSION.en-US.linux-$ARCH.tar.bz2" | grep "sha1" | cut -d" " -f1)
